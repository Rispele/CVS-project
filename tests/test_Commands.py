...
from Commands import InitCommand, AddCommand, CommitCommand, CheckoutCommand, BranchCommand, CreateTagCommand
from cvs import ExecuteInitCommand, ExecuteAddCommand, ExecuteCommitCommand, ExecuteCheckoutCommand,\
...
class TestCommands(TestCase):
    def setUp(self):
        self._test_folder = str(os.getcwd()) + '\\test_repo'
    ...
    def test_init(self):
        ...
        init()
        self.assertEqual(os.path.isdir(f'{self._test_folder}\\.cvs'), True)
        self.assertEqual(os.path.isdir(f'{self._test_folder}\\.cvs\\objects'), True)
        self.assertEqual(os.path.isdir(f'{self._test_folder}\\.cvs\\refs'), True)
        self.assertEqual(os.path.isdir(f'{self._test_folder}\\.cvs\\refs\\heads'), True)
        self.assertEqual(os.path.isdir(f'{self._test_folder}\\.cvs\\refs\\tags'), True)
        self.assertEqual(os.path.isfile(f'{self._test_folder}\\.cvs\\HEAD'), True)
        self.assertEqual(os.path.isfile(f'{self._test_folder}\\.cvs\\LOG'), True)
        self.assertEqual(os.path.isfile(f'{self._test_folder}\\.cvs\\COMMITLOG'), True)


        self.assertEqual(get_file_content(f'{self._test_folder}\\.cvs\\HEAD'),'ref: refs/heads/master')
        self.assertEqual(len(get_file_content(f'{self._test_folder}\\.cvs\\LOG')) > 0, True)
        self.assertEqual(get_file_content(f'{self._test_folder}\\.cvs\\COMMITLOG'), 'Project commits:\n')
    ...
    def test_add(self):
        ...
        self.assertEqual(os.path.isfile(f'{self._test_folder}\\.cvs\\index'), True)
        self.assertEqual(len([p for p in os.walk(self._test_folder + '\\.cvs\\objects')]), 3)
    ...
    def test_checkout_to_commit(self):
        ...
        AddCommand(self._test_folder, '')()
        CommitCommand(self._test_folder, '')()
        edited_file_hash = get_file_content(f'{self._test_folder}\\.cvs\\index').split('\n')[0][-40:]
        CheckoutCommand(self._test_folder, 'd57c3c')()
        file_hash_after_checkout = get_file_content(f'{self._test_folder}\\.cvs\\index').split('\n')[0][-40:]
    ...
    def test_branch(self):
        ...
        last_commit_on_new_branch = get_file_content(f'{self._test_folder}\\.cvs\\refs\\heads\\newbranch')
        last_commit_on_master = get_file_content(f'{self._test_folder}\\.cvs\\refs\\heads\\master')
        self.assertEqual(last_commit_on_master == last_commit_on_new_branch, True)
        ...
        self.assertEqual(last_commit_on_master == last_commit_on_new_branch, False)
